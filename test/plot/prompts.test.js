...
test('default plot spec preset resolves to sparrow-spec-creator skill prompt', () => {
  ...
  expect(preset.systemPrompt).toContain('Do not use `type` inside `animation.enter`')
  expect(preset.systemPrompt).toContain('Supported `ease` values are `linear`, `easeIn`, `easeOut`, and `easeInOut`')
...
test('listPlotSpecPromptPresets() includes minimal and skill-backed presets', () => {
  ...
  expect(minimalPrompt).toContain('animation.enter.preset instead of animation.enter.type')
  expect(minimalPrompt).toContain('Do not use kebab-case values like ease-out')
